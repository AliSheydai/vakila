...
/** Match hiknow sidebar widths (280 / 68) */
const SIDEBAR_WIDTH = '17.5rem'
const SIDEBAR_WIDTH_MOBILE = '18rem'
const SIDEBAR_WIDTH_ICON = '4.25rem'
const SIDEBAR_KEYBOARD_SHORTCUT = 'b'
/** Match hiknow collapse easing */
const SIDEBAR_MOTION =
  'duration-300 ease-[cubic-bezier(0.25,0.8,0.25,1)]'
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          `relative w-(--sidebar-width) bg-transparent transition-[width] ${SIDEBAR_MOTION}`,
      ...
      <div
        ...
        className={cn(
          `fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[inset-inline,width] ${SIDEBAR_MOTION} md:flex`,
          side === 'left'
          ...
          variant === 'floating' || variant === 'inset'
            ...
            : 'group-data-[collapsible=icon]:w-(--sidebar-width-icon) group-data-[side=left]:border-e group-data-[side=left]:border-border/50 group-data-[side=right]:border-s group-data-[side=right]:border-border/50',
          className
        )}
        {...props}
      >
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      size='icon'
      className={cn(
        'size-8 shrink-0 rounded-lg text-muted-foreground transition-all duration-150 hover:bg-muted/80 hover:text-foreground',
        className
      )}
      onClick={(event) => {
        onClick?.(event)
...
function SidebarHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      data-sidebar='header'
      className={cn(
        'relative flex min-h-14 shrink-0 flex-col justify-center border-b border-border/50 px-4 py-2',
        className
      )}
      {...props}
    />
...
function SidebarFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      data-sidebar='footer'
      className={cn(
        'flex shrink-0 flex-col border-t border-border/50 px-2 py-2',
        className
      )}
      {...props}
    />
...
function SidebarContent({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'flex min-h-0 flex-1 flex-col gap-0.5 overflow-auto px-2 group-data-[collapsible=icon]:overflow-hidden',
        className
      )}
      {...props}
    />
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        'flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-sidebar-foreground ring-sidebar-ring outline-hidden hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span]:min-w-0 [&>span]:truncate [&>span]:whitespace-nowrap [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-inherit',
