...
import { PageShell } from '@/features/client-portal/components/page-shell'
import type { ClientSettingsData, UserNotificationPreferences } from './types'
import { ClientMessengersTab } from './components/messengers-tab'
import { ClientNotificationsTab } from './components/notifications-tab'

export function SettingsClientPage() {
  const [data, setData] = useState<ClientSettingsData | null>(null)
  ...
  const loadSettings = useCallback(async () => {
    setLoading(true)
    setError(null)
    const result = await api<ClientSettingsData>('/api/portal/settings')
    setLoading(false)
  ...
  function handleNotificationSaved(settings: UserNotificationPreferences) {
    setData((prev) => {
      if (!prev) return prev
      return { ...prev, notificationPreferences: settings }
    })
  }
      ...
      <div className='mx-auto w-full max-w-2xl space-y-8'>
        <div className='space-y-1'>
          ...
          <p className='text-sm text-muted-foreground md:text-base'>
            اتصال به پیام‌رسان‌ها و نحوه دریافت اعلان‌ها را مدیریت کنید.
          </p>
        </div>
